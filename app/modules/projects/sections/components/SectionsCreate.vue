...
onMounted(async () => {
  ...
  try {
    const data: any = await request(`/api/cms/projects-sections/${sectionId.value}/`, {
      method: "GET",
    })

    form.value = {
      project: data.page ?? null,
...
async function submit() {
  ...
  try {
    ...
    if (isEdit.value && sectionId.value) {
      await request(`/api/cms/projects-sections/${sectionId.value}/`, {
        method: "PATCH",
        body: fd,
      })
      ...
      await request("/api/cms/projects-sections/", {
        method: "POST",
        body: fd,
      })
    ...
    router.push("/cms/projects/sections")
  } catch (e: any) {
    errors.value = e?.data ?? {}
...
async function handleTinyMceImageUpload(blobInfo: any) {
  ...
  const res = await fetch(
    `${apiBase}/api/cms/tinymce/upload-project-section-image/`,
    {
      method: "POST",
      headers: token
        ? {
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold tracking-tight">
          {{ isEdit ? "Edit Page Section" : "Create Projects Section" }}
        </h1>
      ...
      <div class="flex items-center gap-2">
        <Button variant="outline" @click="router.push('/cms/projects/sections')">
          Cancel
        </Button>

        <Button :disabled="loading || !form.project || !form.section_type" @click="submit">
