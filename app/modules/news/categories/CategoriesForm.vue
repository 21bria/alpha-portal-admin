...
} from "@/modules/news/categories/components/DialogForm.vue"

import {
  getNewsCategoryColumns,
  type NewsCategoryRow,
} from "@/modules/news/categories/columns"

import { categoriesConfig } from "@/modules/news/categories/table"
import { categoriesFilters } from "@/modules/news/categories/filters"
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<NewsCategoryRow>>(
  () => `news-categories:${JSON.stringify(query.value)}`,
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    <div class="flex items-center justify-between">
      <div>
        <h3 class="text-lg font-semibold">News Categories</h3>
        <p class="text-sm text-muted-foreground">
          Manage article categories for company portal news.
        </p>
      </div>
    </div>
